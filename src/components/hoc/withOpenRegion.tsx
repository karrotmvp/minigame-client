...
export const withOpenRegion = (WrappedComponent: any, option: boolean) => {
  const OpenRegionCheck = (props: any) => {
    ...
    const checkOpenRegion = useCallback(
      async function (karrotRaiseApi: KarrotRaiseApi, regionId: string | null) {
        try {
          if (regionId === null) {
    ...
    useEffect(() => {
      console.log(code, userRegionId);
      checkOpenRegion(karrotRaiseApi, userRegionId);
